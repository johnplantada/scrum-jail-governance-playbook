...
and no approval message to parse. The same platform primitives you'd use to gate any human
...
described a chat-based variant — a typed approval message parsed by a Registrar bot over
a WebSocket. That mechanism is retired here, not offered as an equally
...
4. THE PLATFORM VERIFIES & ENFORCES
   ...
   └── nothing executes a merged decisions.yaml entry's payload automatically —
       ...
       there is deliberately no code path from "authorization recorded" to
       "money moves". The backstop is still
...
property than a bot-policed convention: GitHub already refuses to let a PR merge without
...
| Agent posts a `[PROPOSAL]` and treats an approving Chairman comment as approval for a money/org-shape ask | Doesn't count — only a merged `decisions.yaml` entry or the Chairman's deploy dispatch is authorization; a `[PROPOSAL]` issue is for asks that aren't money/org-shape in the first place |
