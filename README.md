...
- **Not automated at all:** nothing executes a `decisions.yaml` entry's payload for
  ...
  no code path from a chat-reaction approval in the old model. The hard backstop is still
...
messages, a human reacts in the chat client, a Registrar bot watching the chat
...
bot's behavior. [authorization-gate.md](authorization-gate.md) walks through the GitHub-native
version in full, describing the gate above in detail,
...
chat-based reaction gate and cut over to the GitHub-native model described above on
