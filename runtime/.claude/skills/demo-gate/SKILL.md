...
is `scripts/handoff_check.py` (documented in `agents/_policy.md` §handoffs), enforced from
operator-local compute — never a per-comment hosted workflow (DESIGN.md §4). Treat the
schema as binding even where no automated check fires. Substance is on you; the
