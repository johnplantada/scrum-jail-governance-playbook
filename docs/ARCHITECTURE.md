...
The six governance gates — pure ledger vocabulary (`org-chart.yaml` → `governance:`), not a
signal any bot parses; four of the six are `decisions.yaml` `type` strings, and the other two
are platform/file mechanisms:
...
flowchart TD
    ...
    Pred -->|"shipped = yes"| Active["code review + demo evidence on the product PR<br/>→ merge → the change queues<br/>→ Chairman dispatches deploy.yml (the deploy)"]
```
...
governance gate that was literally a chat approval — the Registrar parsed approval events off
a WebSocket and verified the approver's user id in code. It worked, and every pattern in
...
itself. The before-measurements live in the
